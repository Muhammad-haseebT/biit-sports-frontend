...
import { getTeamRequests, approveTeamRequest, rejectTeamRequest } from "../api/teamApi";
                        ...
                        >
                            <div className="p-4 flex items-center justify-between">
                                <div>
                                    <h3 className="text-lg font-semibold text-gray-900 flex items-center gap-2">
                                        {req.teamName}
                                        {expandedId === req.requestId ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
                                    </h3>
                                    <p className="text-sm text-gray-600">
                                        Captain: <span className="font-medium">{req.CaptainName || "N/A"}</span>
                                    </p>
                                </div>
                            ...
                            {expandedId === req.requestId && (
                                <div className="bg-gray-50 px-4 py-3 border-t border-gray-100">
                                    <h4 className="text-sm font-semibold text-gray-700 mb-2">Team Players:</h4>
                                    {req.players && req.players.length > 0 ? (
                                        ...
                                        <p className="text-sm text-gray-400 italic">No players listed.</p>
                                    )}
                                </div>
                            )}
                        </div>
